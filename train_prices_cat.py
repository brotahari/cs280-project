...
def to_tempfile(file_content):
    ...
    with tempfile.NamedTemporaryFile(delete=False) as f:
        ...
        return f.name
def to_file(file_content, filename):
    with open( filename + time_str, 'w' ) as f:
        f.write(file_content)
...
def minialexnet(data, labels=None, train=False, param=learned_param,
    ...
    n.conv1, n.relu1 = conv_relu(n.data, 11, 96//2, stride=4, **conv_kwargs)
    ...
    preds = n.fc8 = L.InnerProduct(n.drop7, num_output=3, param=param)
    if not train:
    ...
    if with_labels:
        ...
        n.accuracy = L.Accuracy(preds, n.label)
    else:
        n.ignored_label = labels
        n.silence_label = L.Silence(n.ignored_label, ntop=0)

    to_file( str(n.to_proto()), './results/net.prototxt' )
...
def miniplaces_solver(train_net_path, test_net_path=None):
    ...
    to_file( str(s), './results/solverspecs.txt' )
...
def eval_net(split):
    ...
    preds = np.zeros((len(filenames)), dtype = np.float32)
    if known_labels:
    ...
    while offset < len(filenames):
        ...
        for prob in probs:
            preds[offset] = np.argmax( prob )
            if known_labels:
                correct_label_probs[offset] = prob[labels[offset]]
            offset += 1
            if offset >= len(filenames):
                break
    accuracy = 100 * (np.size(preds) - np.size(np.nonzero(preds - labels)))/float(np.size(preds))

    print '\tAccuracy = {0:4.2f}'.format(accuracy)

    
    #with open('./results/preds.json' + time_str, 'w') as f:
        #json.dump(preds,f)
